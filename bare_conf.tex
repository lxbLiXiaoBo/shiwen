...
We consider a single cell scenario where cellular users coexist with D2D users. A set of $\mathcal{M} =
\{1, \ldots, M \}$ cellular users and a set of $\mathcal{N} = \{ 1,\ldots, N \}$ D2D pairs are
distributed uniformly at random within the coverage area of BS.

The uplink (UL) transmission is considered where cellular users and
D2D users share the same number of available RBs, which is denoted by $K$. We assume that each RB is occupied
by one cellular user and can be shared with multiple D2D pairs. There are two kinds of interference in this
scenario due to channel resources sharing. One is that BS receives from D2D transmitters. Another is that the
D2D receivers receive from cellular users and other D2D pairs who share the same resource.
...
The signal to interference plus noise ratio (SINR) should be considered as an important indicator for link quality. We
express the $i$th D2D user' SINR on the $k$th RB as
